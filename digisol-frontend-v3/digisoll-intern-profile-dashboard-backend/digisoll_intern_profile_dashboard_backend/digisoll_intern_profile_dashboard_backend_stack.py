...
class DigisollInternProfileDashboardBackendStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Public (unauthenticated) — sign-up needs to show real department options
        # before the user has any Cognito session to authenticate with.
        departments = api.root.add_resource("departments")
        departments.add_method("GET", integration)
        ...
        # LEAD registers their department at first login — this is what makes a
        # department "real" and selectable by future MEMBER sign-ups.
        dept_register = dept.add_resource("register")
        dept_register.add_method("POST", integration, **auth_opts)

        # MEMBER requests to join their chosen (already-registered) department.
        dept_membership_request = dept.add_resource("membership-request")
        dept_membership_request.add_method("POST", integration, **auth_opts)

        # MEMBER polls their own approval status.
        dept_my_status = dept.add_resource("my-membership-status")
        dept_my_status.add_method("GET", integration, **auth_opts)

        # LEAD reviews and approves/rejects pending join requests for their department.
        dept_pending = dept.add_resource("pending-members")
        dept_pending.add_method("GET", integration, **auth_opts)
        pending_item = dept_pending.add_resource("{userId}")
        pending_item.add_method("PUT", integration, **auth_opts)
